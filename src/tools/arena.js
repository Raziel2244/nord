...
import { useTargetChecked } from '../hooks/useTargetChecked'
...
export function Arena() {
    ...
    const mapSelectOptions = (item) => (<option key={item.level} value={item.level}>{item.title}</option>)

    function handleRoll(e) {
        e.preventDefault();
        console.log({ dragon, horse, battleCry, dragonDrawn, armour, weapon, energy, sneak, hound, trust, bait });
    }

    return (
        <Tool name="arena" title="Arena Roller" onSubmit={handleRoll}>
            <div className="form-row">
                <LabelledInput id="dragon" label="Dragon:" type="select" value={dragon} onChange={setDragon}>
                    {dragonLevels.map(mapSelectOptions)}
                </LabelledInput>
                <LabelledInput id="horse" label="Horse:" type="select" value={horse} onChange={setHorse}>
                    {horseLevels.map(mapSelectOptions)}
                </LabelledInput>
            </div >
            <div className="form-row">
                <LabelledInput id="battleCry" label="Battle cry:" type="checkbox" value={battleCry} onChange={setBattleCry} />
                <LabelledInput id="dragonDrawn" label="Dragon drawn:" type="checkbox" value={dragonDrawn} onChange={setDragonDrawn} />
                <LabelledInput id="armour" label="Enchanted armour:" type="checkbox" value={armour} onChange={setArmour} />
                <LabelledInput id="weapon" label="Enchanted weapon:" type="checkbox" value={weapon} onChange={setWeapon} />
            </div>
            <div className="form-row">
                <LabelledInput id="energy" label="Energy boost:" type="checkbox" value={energy} onChange={setEnergy} />
                <LabelledInput id="sneak" label="Sneak-O-Matic:" type="checkbox" value={sneak} onChange={setSneak} />
                <LabelledInput id="hound" label="Hound familiar:" type="checkbox" value={hound} onChange={setHound} />
                <LabelledInput id="trust" label="Hound trust:" type="select" value={trust} onChange={setTrust}>
                    {trustLevels.map(mapSelectOptions)}
                </LabelledInput>
                <LabelledInput id="bait" label="Live bait:" type="number" value={bait} onChange={setBait} min="0" max="5" step="1" />
            </div>
            <div className="form-row">
