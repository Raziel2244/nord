      ...
      this.entry = {
        ...
        cache: {
          ...
          data: {
            ...
            bazaar: rzl.arrayForCountInObjectsInArrayFilter(
              nord.arena.items,
              "count",
              { key: "group", value: "bazaar" }
            ),
            barracks: rzl.arrayForCountInObjectsInArrayFilter(
              nord.arena.items,
              "count",
    ...
    try {
      ...
      if (fields.sneak.checked) {
        ...
        if (sneak.rng.roll <= sneak.rng.chance) {
          ...
          while (
            typeof sneak.item.season !== "undefined" &&
            !sneak.item.season.month.includes(curMonth)
          ) {
      ...
      switch (fields.trust.value) {
        case "30%":
          cache.rng.chance += 250;
          break;
        case "70%":
          cache.rng.chance += 500;
          break;
        case "100%":
          cache.rng.chance += 750;
          break;
      }
      ...
      if (fields.battlecry.checked) {
        ...
        if (cry.rng.roll <= cry.rng.chance) {
          ...
          if (
            ...
            cache.rng.chance === cache.rng.cap
          ) {
            let bazitem = (cache.items.bazaar =
              nord.arena.items[rzl.randomArrayItem(cache.data.bazaar)]);
            let baritem = (cache.items.barracks =
              nord.arena.items[rzl.randomArrayItem(cache.data.barracks)]);
            while (
              typeof bazitem.season !== "undefined" &&
              !bazitem.season.month.includes(curMonth)
            ) {
            ...
            while (
              typeof baritem.season !== "undefined" &&
              !baritem.season.month.includes(curMonth)
            ) {
      ...
      for (let i = input.effLevel; i > input.dragondata.level; i--)
        battle.rng.chance += 15; // boost per level
      if (fields.armour.checked) battle.rng.chance += 20; // armour boost
      if (fields.weapon.checked) battle.rng.chance += 10; // weapon boost
      if (fields.armour.checked && fields.weapon.checked)
        battle.rng.chance += 15; // armour+weapon combo
      if (input.bait > 0) {
        if (input.bait > 5) {
          input.bait = 5;
        }
        battle.rng.chance += input.bait * 5;
      } // bait bonus
      ...
      if (
        battle.rng.roll <= battle.rng.chance ||
        battle.rng.chance === battle.rng.cap
      ) {
        console.log(`win ${input.dragondata.reward}`);
